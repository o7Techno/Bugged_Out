...
public class ObjectSpawner : MonoBehaviour
{
    ...
    public void GeneratePositionAndSpawn()
    {
        ...
        ItemHeightMap = Noise.GenerateNoiseMap(mapChunkSize + 2, mapChunkSize + 2, WorldData.seed + 1, 35.16f, 5, 0.45f, 2, Vector2.zero, Noise.NormalizeMode.Local);
        for (int i = 0; i < ItemHeightMap.GetLength(0); i++)
        {
            for (int j = 0; j < ItemHeightMap.GetLength(0); j++)
            {
                for (int counter = 0; counter < 3; counter++)
                {
                    if ((ItemHeightMap[i, j] > 0.6f || ItemHeightMap[i, j] < 0.3f))
                    {
                        ...
                        {
                            float x = (float)j / ItemHeightMap.GetLength(0) * 80 * terrainData.uniformScale - 40 * terrainData.uniformScale + Random.Range(-20f, 20f);
                            float z = (float)i / ItemHeightMap.GetLength(0) * 80 * terrainData.uniformScale - 40 * terrainData.uniformScale + Random.Range(-20f, 20f);
